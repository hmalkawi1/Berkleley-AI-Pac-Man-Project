...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            hor, vert = state[0]
            hordir, vertdir = Actions.directionToVector(action)
            visited = state [1]
            nexthor, nextvert = int(hor + hordir), int(vert + vertdir)
            
            if self.walls[nexthor][nextvert] == False:
                
                if (nexthor, nextvert) in self.corners and (nexthor, nextvert) not in visited:
                    successors.append((((nexthor, nextvert), state[1] + [(nexthor, nextvert)]), action, 1))

                else:
                    successors.append((((nexthor, nextvert), state[1]), action, 1))
